...
|---|---|---|
| **SessionStart** | Resolve the binding (honoring a live session override, Phase 5b) and **sync the graph** (blocking — clone/resync a remote, verify a local folder), then inject a combined **confirm + consent primer**: state which graph resolved (`resolution_line()`) and how to switch for this session (`list-graphs` / `use-graph` / `clear-graph-override`) alongside an instruction to ask the user ONCE, up front, whether to use Mnemex this session — if yes, run `opt-in` (records consent, which arms the per-prompt reminder) then `mnx-read` before domain tasks and `mnx-capture` to stage knowledge; if no, run `opt-out` (the command is handed over with this session's id baked in) so Mnemex goes silent for the session. Also emits **nag-only** lines for staged-pending / consolidation-overdue (never auto-runs). If no graph is bound, emit a **one-time onboarding notice** (fires once ever) pointing at `/mnemex:mnx-init` — or, if other graphs are already known (Phase 4 registry), naming them and offering `use-graph` first (pick-or-setup). Silent if the session is already muted. | Sync must be guaranteed before any skill runs; confirming the graph and asking for consent once at turn zero, in the same breath, sets both "which graph" and "should I use it" before any domain work happens, and lets the user drop Mnemex (or point it elsewhere) for the session in one move instead of being nudged every turn. |
